...
export default function PoemsByAuthor() {
  ...
  const poems = useMemo(() => getPoems().filter(p => p.author === author), [author, refreshKey]);
      ...
      <div className="pt-28 pb-20 px-6">
        <div className="max-w-3xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <h1 className="font-display text-4xl font-bold text-foreground mb-2">{label}</h1>
          ...
          <div className="flex flex-wrap gap-3 mb-8">
            ...
            <button
              ...
              className={`px-3 py-2.5 border rounded-lg text-sm font-ui flex items-center gap-1.5 transition-colors ${showFavorites ? 'bg-accent text-accent-foreground border-accent' : 'bg-card border-border text-foreground hover:border-accent/30'}`}
            >
          ...
          <div className="space-y-4">
            {filtered.map((poem, i) => (
              <PoemCard key={poem.id} poem={poem} index={i} onFavoriteToggle={() => setRefreshKey(k => k + 1)} />
            ))}
            {filtered.length === 0 && (
              <p className="text-center font-body text-muted-foreground py-12">No poems found.</p>
            )}
          </div>
        </div>
      </div>
